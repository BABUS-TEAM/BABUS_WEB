	...
	if($noError){
		...
		if($x!=false){
			...
			foreach ($x as $k) {
				...
				$companylogo=urlencode("http://".$serverAddr."/babus/uploads/busvendor_logo/".htmlentities($k->companylogo));
				$companyname=htmlentities($k->companyname);
				$companymoto=htmlentities($k->companymoto);
				$companyaddress=htmlentities($k->companyaddress);
				$companylocation=htmlentities($k->companylocation);
				$company=array("companyname"=>$companyname,"companylogo"=>$companylogo,"companymoto"=>$companymoto,"companyaddress"=>$companyaddress,"companylocation"=>$companylocation);
				array_push($list,$company);
			}
			if(count($list)>1){
				http_response_code(200);
				echo json_encode(array('message'=>$lang['22'],'data'=>$list));
			}else{
				http_response_code(200);
				echo json_encode(array('message'=>$lang['23'],'data'=>$list));				
			}
		}else{
			http_response_code(400);
			echo json_encode(array('message'=>$lang['21']));
		}
	}else{
		http_response_code(400);
