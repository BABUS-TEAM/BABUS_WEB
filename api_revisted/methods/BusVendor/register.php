...
if($noAuthError){
	if($noError){
		$busvendor->companyname = filter_var($data->companyname, FILTER_SANITIZE_STRING);
		$busvendor->companypassword = md5(filter_var($data->companypassword, FILTER_SANITIZE_STRING)); //Hashing the password
		$busvendor->companylogo=$logoName;
		$busvendor->companyaddress = filter_var($data->companyaddress,FILTER_SANITIZE_STRING);
		$busvendor->companylocation = filter_var($data->companylocation, FILTER_SANITIZE_STRING);
		$busvendor->companymoto= filter_var($data->companymoto,FILTER_SANITIZE_STRING);
		$busvendor->companyusername=filter_var($data->companyusername);

		if($busvendor->companyNameExists()){
			http_response_code(400);
			echo json_encode(array('message'=>$lang['14']));
		}else{
			if($busvendor->companyUsernameExists()){
				http_response_code(400);
				echo json_encode(array('message'=>$lang['15']));
			}else{
				if($noPhotoTypeError){
					if($noFileSizeError){
						if($busvendor->register()){
							if($logoName!=''){
									$filename='../../../uploads/busvendor_logo/'.$logoName;
								    move_uploaded_file($x[0],$filename);
							}
							http_response_code(200);
							echo json_encode(array('message' => $lang['16']));
						}else{
							http_response_code(400);
							echo json_encode(array('message' =>  $lang['17']));
						}
					}else{
						http_response_code(200);
