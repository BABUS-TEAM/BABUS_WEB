...
class BusVendor{
	...
	function listBusVendors(){
		$query = "SELECT companyname,companylogo
				FROM " . $this->table_name;
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
		$num = $stmt->rowCount();
	...
	function searchForMatchingBusVendor(){
		$query = "SELECT companyname,companyaddress,companymoto,companylocation,companylogo
				FROM " . $this->table_name ."
	...
	function checkBusVendorNameAndPassword(){
		$query="SELECT * 
		FROM ".$this->table_name." 
