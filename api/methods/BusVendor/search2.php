...
if($noAuthError){
	if($noError){
		$x=$busvendor->searchForMatchingBusVendor();
		if($x!=false){
			foreach ($x as $k) {
				$k->companylogo="http://localhost/babus/uploads/busvendor_logo/".$k->companylogo;
			}
			http_response_code(200);
			echo json_encode(array('message'=>$lang['23'],'data'=>$x));
		}else{
			http_response_code(400);
			echo json_encode(array('message'=>$lang['24']));
		}
	}else{
		http_response_code(400);
