...
if($noAuthError){
	if($noError){
		$x=$trainvendor->listTrainVendors();
		if($x!=false){
			$list=array();
			foreach ($x as $k) {
				...
				$companylogo="http://".$serverAddr."/babus/uploads/trainvendor_logo/".htmlentities($k->companylogo);
			...
			if(count($list)>1){
				...
				echo json_encode(array('message'=>$lang['28'],'data'=>$list));
			}else{
				http_response_code(200);
				echo json_encode(array('message'=>$lang['29'],'data'=>$list));				
			}
		}else{
			http_response_code(400);
			echo json_encode(array('message'=>$lang['27']));
		}
	}else{
		http_response_code(400);
